    ...
    def main():
      ...
      # Visualization 4: Overlay Selected Feature with Monthly Aggregation
        with col1:
            ...
            ax.set_xticks(range(1, 13))
            ax.set_xticklabels(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], fontsize=8)
            ax.set_title(f'Overlay of# Visualization 4: Overlay Selected Feature with Monthly Aggregation
with col1:
    # Grouping data by year and month, then calculating the monthly average for case total and the selected feature
    monthly_data = city_data.groupby(['year', 'month']).agg({
        'case_total': 'sum',
    ...
    ax.set_xticks(range(1, 13))
    ax.set_xticklabels(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], fontsize=8)
    ax.set_title(f'Ave Monthly Cases vs {feature.replace("_", " ").title()}', fontsize=14, color='gray')
    ax.legend(fontsize=8)

    # Displaying the plot
    st.pyplot(fig)
 {feature.replace("_", " ").title()} with Average Monthly Cases', fontsize=14, color='gray')
            ax.legend(fontsize=8)
